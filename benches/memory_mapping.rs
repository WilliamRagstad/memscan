...
fn benchmark_diff_snapshots(c: &mut Criterion) {
    ...
    for size in [1024, 4096, 16384, 65536].iter() {
        let old_snapshot = MemorySnapshot {
            address: 0x10000000,
            data: vec![0xAA; *size],
        };
        ...
        let new_snapshot = MemorySnapshot {
            address: 0x10000000,
            data: new_data,
        };
        ...
        group.bench_with_input(
            BenchmarkId::new("few_changes", size),
            size,
            |b, &_size| {
    ...
    for size in [1024, 4096, 16384, 65536].iter() {
        ...
        group.bench_with_input(
            BenchmarkId::new("many_changes", size),
            size,
            |b, &_size| {
                b.iter(|| diff_snapshots(black_box(&old_snapshot), black_box(&new_snapshot)));
            },
    ...
    for size in [1024, 4096, 16384, 65536].iter() {
        ...
        group.bench_with_input(
            BenchmarkId::new("no_changes", size),
            size,
            |b, &_size| {
                b.iter(|| diff_snapshots(black_box(&old_snapshot), black_box(&new_snapshot)));
            },
