...
//!
//! This benchmarks both naive_search and optimized_search functions which are critical for performance
...
fn benchmark_optimized_search(c: &mut Criterion) {
    let mut group = c.benchmark_group("optimized_search");
    ...
    for size in [1024, 4096, 16384, 65536].iter() {
        ...
        // Benchmark: Pattern not found (worst case - full scan)
        group.bench_with_input(BenchmarkId::new("miss_short", size), size, |b, &_size| {
            b.iter(|| optimized_search(black_box(&haystack), black_box(pattern_short)));
        });

        group.bench_with_input(BenchmarkId::new("miss_medium", size), size, |b, &_size| {
            b.iter(|| optimized_search(black_box(&haystack), black_box(pattern_medium)));
        });

        group.bench_with_input(BenchmarkId::new("miss_long", size), size, |b, &_size| {
            b.iter(|| optimized_search(black_box(&haystack), black_box(pattern_long)));
        });
    }
    ...
    group.bench_function("hit_beginning", |b| {
        b.iter(|| optimized_search(black_box(&haystack_with_pattern), black_box(pattern)));
    });

    group.bench_function("hit_middle", |b| {
        b.iter(|| {
            // Start searching from position that would find middle pattern first
            optimized_search(black_box(&haystack_with_pattern[100..]), black_box(pattern))
        });
    });
...
fn benchmark_optimized_search_realistic(c: &mut Criterion) {
    let mut group = c.benchmark_group("optimized_search_realistic");
    ...
    for (name, pattern) in patterns.iter() {
        group.bench_with_input(
            ...
            pattern,
            |b, &pattern| {
                b.iter(|| optimized_search(black_box(&realistic_page), black_box(pattern)));
            },
        );
    }
